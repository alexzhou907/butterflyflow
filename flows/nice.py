...
from .weight_norm import Conv2dWeightNorm, NIN2d, NIN4d
from .attention import MultiHeadAttention2d
...
class NICE(Flow):
    ...
    def calc_mu_and_scale(self, z1: torch.Tensor, s=None):
        ...
        scale = None
        if self.scale:
            mu, log_scale = torch.chunk(mu, 2, dim=1)
    ...
    def backward(self, input: torch.Tensor, s=None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.scale:
            ...
            logdet = scale.log().reshape(z1.size(0), -1).sum(dim=1) * -1.0
        else:
            logdet = z1.new_zeros(z1.size(0))
